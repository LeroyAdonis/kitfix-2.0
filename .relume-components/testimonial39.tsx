...
const LogoCard = ({ logo }: LogoCardProps) => (
  <Card className="flex items-center justify-center p-6 md:p-8 lg:p-6">
...
const QuoteCard = ({
  ...
  <Card
    className={clsx(
      "order-first flex flex-col items-start justify-between p-6 sm:col-span-2 md:p-8 lg:order-none",
      className,
    )}
  >
    <div className="mb-5 flex gap-1 md:mb-6">
      {Array(numberOfStars)
        .fill(null)
        .map((_, index) => (
          <StarFull key={index} className="size-6 text-scheme-text" />
        ))}
    </div>
