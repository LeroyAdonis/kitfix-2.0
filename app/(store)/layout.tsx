...
export default async function StoreLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-surface-deep">
      ...
      <header className="sticky top-0 z-50 border-b border-white/[0.03] bg-surface-deep/90 backdrop-blur-md">
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-6">
            <Link
              href="/shop"
              className="flex items-center gap-2 text-xs font-semibold tracking-wider uppercase text-white/80 hover:text-green-400 transition-colors duration-300 group"
            >
            ...
            <nav className="hidden items-center gap-1 md:flex">
              <Link
                href="/shop"
                className="relative px-3 py-2 text-xs font-medium tracking-widest uppercase text-content-secondary transition-colors duration-300 hover:text-green-400 group"
              >
                All Products
